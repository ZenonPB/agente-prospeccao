...
@router.get("")
def list_prospect_lists(
    db: Session = Depends(get_db),
    _member: OrganizationMember = Depends(require_analyst()),
    org: Organization = Depends(get_user_organization),
):
    rows = db.query(ProspectList).filter(
        ProspectList.organization_id == org.id,
        ProspectList.active.is_(True),
    ).order_by(ProspectList.name.asc()).all()
    counts = dict(
        db.query(ProspectListMember.list_id, __import__("sqlalchemy").func.count(ProspectListMember.id))
        .filter(ProspectListMember.organization_id == org.id)
        .group_by(ProspectListMember.list_id).all()
    )
    return {"items": [_list_dict(row, int(counts.get(row.id, 0))) for row in rows]}


@router.post("", status_code=status.HTTP_201_CREATED)
def create_prospect_list(
    ...
    existing = db.query(ProspectList).filter(
        ProspectList.organization_id == org.id,
...
def add_member(
    ...
    existing = db.query(ProspectListMember).filter(
        ProspectListMember.list_id == target.id,
        ProspectListMember.lead_id == lead.id,
    ).first()
    if existing:
        return {"id": str(existing.id), "list_id": str(target.id), "lead_id": str(lead.id), "created": False}
    row = ProspectListMember(
        organization_id=org.id,
        list_id=target.id,
        lead_id=lead.id,
        source="manual",
...
def list_members(
    ...
    if not target:
        raise HTTPException(status_code=404, detail="Lista não encontrada")
    rows = db.query(ProspectListMember, Lead).join(Lead, Lead.id == ProspectListMember.lead_id).filter(
        ProspectListMember.organization_id == org.id,
        ProspectListMember.list_id == target.id,
        Lead.organization_id == org.id,
    ).order_by(ProspectListMember.created_at.desc()).limit(limit).all()
    return {"items": [{
        "id": str(member.id),
        "lead_id": str(lead.id),
        "company_name": lead.company_name,
        "status": lead.status.value if getattr(lead.status, "value", None) else str(lead.status),
