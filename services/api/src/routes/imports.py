...
def confirm_import(
    ...
    """Confirma a importação; o replay idempotente é resolvido no service.

    A rota não reinterpreta conflito como sucesso: ``confirm`` já devolve o
    próprio job quando o request é o retry idempotente da confirmação vencedora
    e reserva a transição com compare-and-swap quando é participante da corrida.
    """
    try:
        _assert_job_scope(get_job(db, _org.id, import_id), user, _member)
        job = confirm(
