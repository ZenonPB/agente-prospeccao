  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          {(certifications.data?.items.length ?? 0) === 0 ? <EmptyState title="Nenhuma conta real certificada" description="Depois de configurar uma credencial válida, use “Certificar conta real”. A ausência de certificação não é tratada como aprovação." /> : <div className="space-y-2">{certifications.data?.items.slice(0, 12).map((run) => <div key={run.id} className="flex flex-col gap-2 rounded-lg border p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{PROVIDER_LABELS[run.provider]}</p><p className="text-sm text-muted-foreground">{formatDate(run.created_at)} · versão {run.adapter_version} · {run.checks.length} verificação(ões)</p></div><Badge variant={run.status === 'PASSED' ? 'secondary' : 'destructive'}>{run.status === 'PASSED' ? 'Certificada' : 'Falhou'}</Badge></div>)}</div>}
        </CardContent>
      </Card>
