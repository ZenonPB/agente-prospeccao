...
import { CheckCircle2, Loader2, PlugZap, RefreshCw, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <Card className="border-dashed">
        <CardContent className="flex gap-3 p-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">A chave informada é criptografada e nunca volta para a tela. A sincronização só funciona depois de ser ativada explicitamente.</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <Card>
          <CardHeader><CardTitle>Integrações configuradas</CardTitle><CardDescription>Confira conexão e última atualização sem expor a credencial armazenada.</CardDescription></CardHeader>
            ...
            {(connections.data?.items.length ?? 0) === 0 ? <EmptyState title="Nenhuma integração configurada" description="Conecte o sistema comercial usado pela equipe quando quiser sincronizar os dados." /> : connections.data?.items.map((connection) => (
              <article key={connection.id} className="rounded-xl border p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  {connection.last_health_status === 'ok' ? <><CheckCircle2 className="size-4 text-primary" aria-hidden="true" />Conexão verificada</> : <span>Conexão ainda não verificada</span>}
                </div>
