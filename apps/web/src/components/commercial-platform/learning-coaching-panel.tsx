...
export function LearningCoachingPanel() {
  ...
  const [approvalEvidence, setApprovalEvidence] = useState('Comparação com resultados atribuídos e amostra mínima atendida.');
  ...
  const create = async (offerKey: string) => {
    try {
      ...
      setComparison(value);
      toast.success(`Comparação ${value.version_a} → ${value.version_b} criada com histórico de resultados.`);
    } catch (error) {
  ...
  return <div className="space-y-6">
    <Card>
      <CardHeader>
        <div className="flex items-start gap-3"><BrainCircuit className="mt-1 size-5 text-primary" aria-hidden="true" /><div><CardTitle>Calibração das avaliações</CardTitle><CardDescription>Mudanças na forma de avaliar só são sugeridas com amostra suficiente e resultado comprovado. Nada entra em vigor automaticamente.</CardDescription></div></div>
      </CardHeader>
      <CardContent className="space-y-4">
        {calibration.isLoading ? <Loading label="Calculando amostras e associações..." /> : calibration.isError ? <EmptyState title="Não foi possível calcular a calibração" description="Os dados comerciais continuam preservados; tente recarregar a página." /> : !calibration.data?.items.length ? <EmptyState title="Ainda não há histórico suficiente para calibrar" description="Aparecerão aqui as ofertas com oportunidades ou resultados registrados." /> : calibration.data.items.map((item) => <article key={item.offer_key} className="rounded-xl border p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Badge variant={item.sample_quality.eligible_for_proposal ? 'secondary' : 'outline'}>{item.sample_quality.eligible_for_proposal ? 'Pronta para propor' : 'Amostra insuficiente'}</Badge>
          </div>
          ...
          {canManage ? <div className="mt-4"><Button disabled={!item.sample_quality.eligible_for_proposal || createComparison.isPending} onClick={() => void create(item.offer_key)}><Sparkles className="mr-2 size-4" />Gerar proposta e comparar</Button></div> : null}
        </article>)}
      </CardContent>
    </Card>

    {comparison ? <Card className="border-primary/30">
      <CardHeader><CardTitle>Comparação {comparison.version_a} → {comparison.version_b}</CardTitle><CardDescription>{comparison.result.recommendation}</CardDescription></CardHeader>
      <CardContent className="space-y-4">
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Propostas aguardando publicação</CardTitle><CardDescription>Publicar ativa a versão aprovada.</CardDescription></CardHeader><CardContent className="space-y-3">{proposals.isLoading ? <Loading label="Carregando propostas..." /> : !proposals.data?.proposals.length ? <EmptyState title="Nenhuma proposta pendente" description="Quando uma comparação conclusiva for aprovada, ela aparecerá aqui." /> : proposals.data.proposals.map((proposal) => <div key={proposal.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div><p className="font-medium">{offerProfileLabel(proposal.offer_key)} · {proposal.approved_version}</p><p className="text-sm text-muted-foreground">Proposta #{proposal.proposal_version} · {proposal.status}</p></div>{canManage && proposal.status === 'PROPOSED' ? <Button disabled={publish.isPending} onClick={() => publish.mutate(proposal.id, { onSuccess: () => toast.success('Nova versão publicada.'), onError: (error) => toast.error(error instanceof Error ? error.message : 'Falha ao publicar.') })}>Publicar versão aprovada</Button> : <Badge variant="outline">{({ 'PROPOSED': 'Aguardando aprovação', 'PUBLISHED': 'Publicada', 'REJECTED': 'Recusada' } as Record<string, string>)[proposal.status] ?? proposal.status}</Badge>}</div>)}</CardContent></Card>
      <Card><CardHeader><CardTitle>Histórico de versões</CardTitle><CardDescription>Voltar para uma versão anterior reativa critérios já aprovados, sem recalcular nada.</CardDescription></CardHeader><CardContent className="space-y-3">{versions.isLoading ? <Loading label="Carregando versões..." /> : !versions.data?.items.length ? <EmptyState title="Sem versões publicadas" description="Vale a regra padrão do catálogo enquanto a equipe não publicar uma versão." /> : versions.data.items.map((version) => <div key={version.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border p-4"><div className="flex items-center gap-2"><History className="size-4 text-muted-foreground" /><div><p className="font-medium">{offerProfileLabel(version.offer_key)} · {version.version}</p><p className="text-sm text-muted-foreground">{version.is_active ? 'Ativa agora' : 'Versão anterior'}</p></div></div>{version.is_active ? <Badge variant="secondary">Ativa</Badge> : canManage ? <Button variant="outline" disabled={rollback.isPending} onClick={() => rollback.mutate({ offerKey: version.offer_key, targetVersion: version.version }, { onSuccess: () => toast.success('Versão anterior reativada.'), onError: (error) => toast.error(error instanceof Error ? error.message : 'Não foi possível reativar a versão.') })}><RotateCcw className="mr-2 size-4" />Reativar</Button> : null}</div>)}</CardContent></Card>
    </div>

    <Card>
      <CardHeader><div className="flex items-start gap-3"><UsersRound className="mt-1 size-5 text-primary" /><div><CardTitle>Desempenho por consultor</CardTitle><CardDescription>Diagnóstico por consultor usando atividades, resultados e prazos reais. Recomendações deixam explícito quando são apenas coincidências nos dados.</CardDescription></div></div></CardHeader>
      <CardContent className="space-y-4">{coaching.isLoading ? <Loading label="Analisando operação do time..." /> : coaching.isError || !coaching.data ? <EmptyState title="Não foi possível montar a análise" description="Tente novamente em alguns instantes." /> : <>
        <div className="grid gap-3 sm:grid-cols-4"><Metric label="Leads atribuídos" value={String(coaching.data.team.assigned_leads)} /><Metric label="Reuniões" value={String(coaching.data.team.meetings)} /><Metric label="Vendas" value={String(coaching.data.team.wins)} /><Metric label="Contatos vencidos" value={String(coaching.data.team.overdue_followups)} /></div>
        {!coaching.data.consultants.length ? <EmptyState title="Ainda não há responsáveis com carteira" description="A análise por consultor aparece quando houver leads sob responsabilidade." /> : coaching.data.consultants.map((consultant) => <article key={consultant.user_id} className="rounded-xl border p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-medium">{consultant.name}</h3><p className="text-sm text-muted-foreground">{consultant.contacted} contatos · {consultant.meetings} reuniões · {consultant.proposals} propostas · {consultant.wins} vendas</p></div><Badge variant={consultant.recommendations.length ? 'secondary' : 'outline'}>{consultant.recommendations.length ? `${consultant.recommendations.length} ponto(s) de atenção` : 'Sem alerta com amostra suficiente'}</Badge></div>{consultant.recommendations.map((recommendation) => <div key={recommendation.kind} className="mt-3 rounded-lg bg-muted/40 p-3"><p className="font-medium">{recommendation.title}</p><p className="mt-1 text-sm">{recommendation.recommendation}</p><p className="mt-1 text-xs text-muted-foreground">Baseado em contagens reais desta equipe. Coincidência observada; não prova que uma coisa causa a outra.</p></div>)}</article>)}
      </>}</CardContent>
    </Card>
  </div>;
}
