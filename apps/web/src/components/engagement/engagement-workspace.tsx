...
export function EngagementWorkspace() {
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-3 sm:grid-cols-3" aria-label="Resumo de engagement">
        ...
        <MetricCard icon={<Workflow className="size-4" aria-hidden="true" />} label="Workflows ativos" value={enabledWorkflows} />
      </section>
...
function ConditionEditor({ value, onChange, index }: { value: Record<string, unknown>; onChange: (value: Record<string, unknown>) => void; index: number }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-3"><div className="space-y-2"><Label htmlFor={`condition-field-${index}`}>Dado</Label><Select value={field} onValueChange={(next) => onChange({ ...value, field: next })}><SelectTrigger id={`condition-field-${index}`} className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="lead.status">Status do lead</SelectItem><SelectItem value="lead.score">Pontuação</SelectItem><SelectItem value="lead.opt_out">Opt-out</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label htmlFor={`condition-value-${index}`}>Valor esperado</Label><Input id={`condition-value-${index}`} value={String(value.value ?? '')} onChange={(event) => onChange({ ...value, value: event.target.value, operator })} /></div><div className="space-y-2"><Label>Se não atender</Label><Select value={onFalse} onValueChange={(next) => onChange({ ...value, on_false: next, operator })}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="skip">Pular etapa</SelectItem><SelectItem value="stop">Encerrar sequência</SelectItem></SelectContent></Select></div></div>;
}
...
function WorkflowSection() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const config = action === 'CREATE_TASK' ? { title: 'Revisar oportunidade acionada pelo workflow', task_type: 'RESEARCH' } : action === 'NOTIFY' ? { title: 'Nova oportunidade requer atenção' } : {};
    try {
...
function RunSection({ loading, items }: { loading: boolean; items: Array<{ id: string; trigger_type: string; event_key: string; status: string; started_at?: string | null; action_results: Array<Record<string, unknown>> }> }) {
  ...
  if (!items.length) return <EmptyState title="Nenhuma execução registrada" description="O histórico aparecerá aqui quando um workflow receber um evento elegível." />;
