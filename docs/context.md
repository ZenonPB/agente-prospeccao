...
**Bugfix de responsividade mobile — overflow horizontal em KPIs/cards:**
...
- `apps/web/src/app/(protected)/layout.tsx`: root `flex h-dvh w-full max-w-full
  ...
  `min-w-0 max-w-full` — em flex children sem `min-w-0` o `overflow-hidden`
  do pai não segura o conteúdo, e o kanban forçava 7×300px = 2100px de
  largura fazendo a página inteira rolar lateralmente.
- `components/layout/header.tsx`: `header` ganha `min-w-0 shrink-0 gap-2`
  para o título "Prospect.ai" do mobile não tracionar o flex.
- `components/vendas/kanban-board.tsx`: container do kanban com
  ...
  rounded-xl` envolvendo um `flex w-max gap-3 px-1` — isola o scroll
...
**Marketing/SEO/acessibilidade — checklist de site institucional:**
- Página 404 personalizada em `app/not-found.tsx` (visitante sem login) e
  `app/(protected)/not-found.tsx` (autenticado, mostra atalhos para áreas
  conhecidas). `metadata.robots = { index: false }` em ambas — privadas.
- `app/robots.ts` (Next MetadataRoute) bloqueia tudo exceto login, register,
  ...
  origin. `app/sitemap.ts` lista só as URLs públicas acima (prioridade alta
  para login/register, baixa para fluxos de token).
- `app/layout.tsx`: `metadataBase` (env `NEXT_PUBLIC_SITE_URL`), `title`
  com `default` + `template: "%s · Prospect.ai"`, descrição canônica,
  OpenGraph/Twitter pt-BR, `robots: { index: false }` por padrão (app
  interno) — públicas auth sobrescrevem.
- `lib/metadata.ts` (novo): `buildPageMetadata({ title, description, noindex })`
  para padronizar títulos/descrições/OG/Twitter em todas as pages.
- Layouts server por segmento (`app/(protected)/{campanhas,oportunidades,
  ...
  `vertentes`/`consultores`) exportam `metadata` único para a rota, herdado
...
- Alt text: `brand-logo.tsx` e `auth-shell.tsx` (4 ocorrências) trocam
  "AlphaMec Logo" por "Logotipo da AlphaMec"; fotos do carrossel da
  auth-shell mantêm `alt={photo.caption}`. `not-found.tsx` ícone + 404
  interno idem (ícones decorativos marcados com `aria-hidden`). Página de
  configurações: `aria-hidden` em `AvatarFallback` (iniciais decorativas) e
  ícones de botões, `aria-label` nos botões de tema, `aria-pressed` em
  toggle de mostrar/ocultar senha e cards de tema.
- Página `/ajuda` (nova) com 3 seções ancoradas (`#faq`, `#depoimentos`,
  `#privacidade`) — FAQ em `<details>`/`<summary>` (HTML semântico, indexa
  no Google), 3 cards de depoimentos e bloco de política de privacidade com
  6 seções. Conteúdo marcado explicitamente com `TODO:` para o time
  revisar/preencher antes de publicar. Sidebar ganha grupo "Suporte" com
  link para `/ajuda`. `metadata` específica (noindex).
- CTA acima da dobra: `ProcessBanner` no `/dashboard` já exibe botão
  "Nova Busca Rápida" no topo do viewport, antes de qualquer scroll —
  ratificado sem alterações.
- Mobile/bottom-nav: a entrada `/ajuda` aparece no sidebar e respeita o
  `lg:hidden` do `MobileBottomNav` (visível no celular) com a regra de
...
- `npx tsc --noEmit`/`lint`/`build` limpos. `ts-prune` indica 5 candidatos
  ...
  - `components/pipeline/pipeline-monitor.tsx:PipelineMonitor` — rota
    `/pipeline` removida na unificação; órfão.
  - `components/oportunidades/version-history.tsx:VersionHistoryButton` —
    export não consumido (importação interna: nenhum).
  - `components/layout/org-switcher.tsx:getActiveOrgId` — export não
    consumido; manter se for API pública do módulo.
  - `components/relatorios/brazil-state-map.tsx:GeoCardError`,
    `components/relatorios/list-cards.tsx:ListCardError` —
    provavelmente cobertos por `ChartCardError` em
    `relatorios/chart-cards.tsx` (já usado no `RelatoriosPage`).
...
### Renova��o UI/UX � "Radar Vivo" (frontend)

Redesign da camada visual do `apps/web` sem mudan�as de rotas, dados ou contratos de API.
Sistema de movimento novo em `globals.css` (keyframes `fade-up/scale-in/sonar/sheen/shimmer/float`,
eleva��es `--shadow-soft/lift` via `color-mix`, utilit�rios `.card-lift/.btn-sheen/.sonar/.radar-dot/
...
- **Command palette** novo: `components/layout/command-menu.tsx` (Ctrl/?+K ou evento
  `open-command-menu`) � navega��o global respeitando permiss�o de Relat�rios; gatilhos no `header.tsx`.
- **`(protected)/layout.tsx`** envolve children em `PageTransition` (entrada suave por rota).
- `page-header.tsx` com eyebrow pontilhado + sublinhado gradiente; `empty-state.tsx` com an�is sonar;
  `skeleton.tsx` com shimmer; `button.tsx` variant default com sheen; `card.tsx` com sombra suave default.
- `sidebar.tsx`: indicador ativo animado, �cones com zoom no hover, `sr-only`/`title` no modo
  recolhido (a11y), footer com status "Radar operando".
- Dashboard: `metrics.tsx` com n�meros animados, chips de cor por m�trica, card-lift, teclado
  (role=button + Enter/Espa�o) e copy sem jarg�o ("Prontas para contato", "bem avaliadas pela IA");
  `process-banner.tsx` com auroras, passos em cascata e copy simples ("A IA avalia", "Fa�a o contato").
...
- Validado: `npm run lint` limpo, `tsc --noEmit` limpo, `npm run build` OK (18 rotas), `/login`
  renderiza 200 em dev. QA visual por screenshot ficou pendente (browser automation n�o subiu a tempo).

Pr�ximo passo sugerido: aplicar `Reveal`/stagger nas demais p�ginas (relatorios, vendas kanban,
oportunidades/[id]) e revisar linguagem dos cards internos (playbooks, SLA, templates).

### Tour mais fluido + Kanban arrast�vel de verdade (frontend)

**Tutorial (`guided-tour-manager.tsx`, `tour-steps.ts`, `tour-styles.css`):**
- Boas-vindas e encerramento agora s�o passos **centralizados** (`elementSelector: null`), sem elemento.
- Indicador "Preparando a pr�xima parada�" (p�lula fixa com spinner + contador) durante a
  espera de elemento/navega��o entre paradas � cobre o v�cuo do overlay.
- Barra de progresso real injetada no popover (`onPopoverRender`, insere `.agente-tour-progress`
  antes do footer) + anima��o de entrada do popover + estilo do bot�o Voltar desabilitado.
- Toast de celebra��o ao concluir. Copy de todas as 20 etapas reescrita: 1�2 frases diretas
  ("o que � + o que fazer"), sem jarg�o (SLA ? "lead parado volta ao radar", etc.).

**Kanban (`kanban-board.tsx`):**
- Al�a de arraste ampliada: agora � a **linha inteira do topo do cart�o** (puxador maior + nome
  + nota), com `touch-none` (mobile arrasta sem quebrar o scroll do corpo) e cursor grab/grabbing.
  O corpo do cart�o continua livre para clique/bot�es.
- Feedback de arraste: `onDragStart` guarda a coluna de origem ? colunas-alvo ganham borda
  tracejada vis�vel, cart�es n�o arrastados ficam esmaecidos (`opacity-40 saturate-50`), board
  com cursor grabbing; cart�o arrastado com ring mais forte.
- Textos de ajuda atualizados ("Segure o topo de um cart�o e arraste�").
- Validado: lint, tsc e build limpos.

### Reveal/stagger nas telas restantes + revis�o de copy (frontend)
...
- **Reveal/stagger estendido**: `relatorios/page.tsx` (6 blocos com delays incrementais 0�350ms),
  ...
  delay 60 + tabs com animate-fade-up stagger-2). Dashboard e p�ginas base j� cobertos antes.
- **Copy de configura��es**: varrido todos os cards (`configuracoes/*.tsx`, `vertentes/*.tsx`).
  A copy j� estava majoritariamente em linguagem simples (trabalho anterior: "Nota m�nima",
  "Prazos de atendimento", "Modelos de Mensagens"). �nico jarg�o restante corrigido em
