...
| Snapshot append-only de oportunidades (`lead_opportunity_snapshots`) com hash canônico e vínculo de venda ao snapshot | A linha atual de `lead_opportunities` é mutável por re-scoring; sem histórico, publicar uma nova versão do perfil reescreveria o contexto de vendas antigas. Snapshot idempotente por `(lead_id, snapshot_hash)` preserva auditoria. |
| Re-scoring preserva versão sem reavaliação explícita (`explicit_reanalyze`) | Oportunidades existentes não mudam de versão automaticamente; novas coletas usam a versão nova e campanha ativa só migra via `reanalyze` explícito. Evita reescrita silenciosa do funil. |
