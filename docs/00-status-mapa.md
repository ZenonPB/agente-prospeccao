...
| Company 360 read-only | ✅ | backend + frontend + testes PostgreSQL. |
| Person 360 read-only | ✅ | backend + frontend + testes PostgreSQL. |
...
3. Validar CRM/BI/coaching com o fluxo comercial real da AlphaMec.
4. Rodar campanhas reais autorizadas para medir precisão e conversão por oferta.

## Próximo corte do RC

CRM operacional/BI avançado → coaching/calibração → UAT multi-workspace → campanha real autorizada/hardening final.
