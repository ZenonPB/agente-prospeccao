...
| Event Discovery | ✅ opt-in | `source=events` no pipeline | `event_opportunities` | não dispara oferta/decisor/outreach |
...
| Expiração de eventos | 🟠 | scheduler da API | status `upcoming/expired` | TTL explícito em string precisa conversão correta |
| Intent Engine | 🟠 | enrichment com HTML/jobs fornecidos | `lead.evidence_score.phase3` | falta job board/producer real |
| Decision Maker Resolution | 🟠 | `ContactEnrichmentService` | snapshot JSONB + entidades parciais | pipeline canônico de `Person` incompleto |
...
| Observabilidade agregada | 🟠 | logs/jobs e status de providers | logs; sem série agregada | faltam duração, custo, quota e empty rate por provider |
...
1. P0.5 — métricas agregadas de provider, duração, erros, empty rate, quota e
   custo.
2. P1.17 — evento → `OfferMatcher` → `LeadOpportunity`.
